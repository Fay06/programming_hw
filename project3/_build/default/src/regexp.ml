...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  match regexp with
    ...
    let first = fresh() in 
      let last = fresh() in  
        {sigma = []; qs = [first; last]; q0 = first; fs = [last]; delta = [(first, None, last)]}
    | Char c ->
    let first = fresh() in 
      let last = fresh() in  
        {sigma = [c]; qs = [first; last]; q0 = first; fs = [last]; delta = [(first, Some c, last)]}
    | Union (r1, r2) ->
    let first = fresh() in 
      let last = fresh() in
        let a = regexp_to_nfa r1 in
          let b = regexp_to_nfa r2 in
            {sigma = Sets.union a.sigma b.sigma; 
            qs = (Sets.union a.qs b.qs)@[first; last]; 
            q0 = first; fs = [last]; 
            delta = (Sets.union a.delta b.delta)
              @[(first, None, a.q0); (first, None, b.q0)]
              @(List.fold_left (fun acc ele -> (ele, None, last)::acc) [] a.fs)
              @(List.fold_left (fun acc ele -> (ele, None, last)::acc) [] b.fs)}
    | Concat (r1, r2) ->
    let a = regexp_to_nfa r1 in
      let b = regexp_to_nfa r2 in
        {sigma = Sets.union a.sigma b.sigma; 
        qs = Sets.union a.qs b.qs; q0 = a.q0; fs = b.fs; 
        delta = (Sets.union a.delta b.delta)@(List.fold_left (fun acc ele -> (ele, None, b.q0)::acc) [] a.fs)}
    | Star r ->
    let first = fresh() in 
      let last = fresh() in
        let a = regexp_to_nfa r in
          {sigma = a.sigma; qs = a.qs@[first; last]; q0 = first; fs = [last]; 
            delta = a.delta@[(first, None, last); (last, None, first); (first, None, a.q0)]@(List.fold_left (fun acc ele -> (ele, None, last)::acc) [] a.fs)}
