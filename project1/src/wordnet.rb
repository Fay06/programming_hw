...
class CommandParser
    ...
    def parse(command)
        ...
        when "load"
            synset = Synsets.new
            hypernym = Hypernyms.new
            hash[:recognized_command] = :load
            if com.length() != 3 or !com[1] =~ /[\w\_\-\.\'\/]+/ or !com[2] =~ /[\w\_\-\.\'\/]+/
                hash[:result] = :error
            elsif !synset.load(com[1]).nil? or !hypernym.load(com[2]).nil?
                hash[:result] = false
            elsif hypernym.load_command(com[2], @synsets) and hypernym.load_command(com[2], synset)
                s1 = @synsets.get
                s2 = synset.get
                s1.each do |k,v|
                    if s2.include?(k)
        ...
        when "find"
            ...
            if com.length() != 2 or !com[1] =~ /[\w\_\-\.\'\/]+/ or @synsets.findSynsets(com[1]).nil?
                hash[:result] = :error
            else
        ...
        when "lca"
            ...
            if com.length() != 3 or com[1].to_i < 0 or com[2].to_i < 0
                hash[:result] = :error
            else
